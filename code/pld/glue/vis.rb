...
#
# As well as the vcd file, this will also generate a text containing
# all $defines from the input file, which can then be used as a 
# translation filter file in GTKWave (right click signal, data format,
# translation filter file, select and enable. This is buggy on macOS
# but does work if you drive the UI correctly).
#
#
require 'date'
#require 'pp'
...
### Write GTK Wave translation filter file for 'enums'
input = File.read("#{basename}.pld")
enums = input.scan(/\$define\s+([^\s]+)\s+'([bhD])'([01]+)/)
  .map { |(name, base, num)| [name, num.to_i(BASE_CONV[base] || 10)] }
  .sort_by { |(name, num)| num }
  .map { |(name, num)| "#{'%02X' % num} #{if ENABLE_COLORS then '?' + COLORS.shuffle.first + '?' else '' end}#{name}" }

unless enums.empty?
  gtkw_xlat = File.open("gtkw-enums.txt", "w") do |f|
    f.puts("# Generated by vis script")
    enums.each { |e| f.puts(e) }
  end
end
